...
void SpiInit(Spi_t *obj)
{
    ...
    if(obj->Spi == (SPI_TypeDef *)SPI2_BASE) {
...
uint16_t SpiInOut( Spi_t *obj, uint16_t outData )
{
    ...
    ((SPI_TypeDef *)(obj->Spi))->DR = outData; //Пишем в буфер передатчика SPI
    while(!(((SPI_TypeDef *)(obj->Spi))->SR & SPI_SR_RXNE)); //Ожидаем окончания приема данных модулем SPI1 (RXNE =1 - приемный буфер содержит данные)
    return ((SPI_TypeDef *)(obj->Spi))->DR; //Считываем данные из приемного буфера SPI При этой операции происходит очистка буфера и сброс флага RXNE
}
