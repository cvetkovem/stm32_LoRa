...
void SpiInit(Spi_t *obj)
{
    ...
    ((GPIO_TypeDef *)(AHB1PERIPH_BASE + 0x400*(obj->Miso.portIndex)))->AFR[(obj->Miso.pinIndex < 8)?0:1] |= 0x05 << 4*(obj->Miso.pinIndex - ((obj->Miso.pinIndex < 8)?0:8));
    ((GPIO_TypeDef *)(AHB1PERIPH_BASE + 0x400*(obj->Mosi.portIndex)))->AFR[(obj->Mosi.pinIndex < 8)?0:1] |= 0x05 << 4*(obj->Mosi.pinIndex - ((obj->Mosi.pinIndex < 8)?0:8));
    ((GPIO_TypeDef *)(AHB1PERIPH_BASE + 0x400*(obj->Sclk.portIndex)))->AFR[(obj->Sclk.pinIndex < 8)?0:1] |= 0x05 << 4*(obj->Sclk.pinIndex - ((obj->Sclk.pinIndex < 8)?0:8));

    if(obj->slave_on) {
        GpioInit(&obj->Nss,  PIN_ALTERNATE_FCT, PIN_PUSH_PULL, PIN_PULL_UP, 0);
        ((GPIO_TypeDef *)(AHB1PERIPH_BASE + 0x400*(obj->Nss.portIndex)))->AFR[(obj->Nss.pinIndex  < 8)?0:1] |= 0x05 << 4*(obj->Nss.pinIndex - ((obj->Nss.pinIndex < 8)?0:8));
    } else {
...
void SpiDeInit(Spi_t *obj)
{
    ...
    if(spi == SPI1) {
        ...
        *((uint32_t *)(gpio_reg + 24)) = 0x00;
        /* RCC->APB2ENR &= ~RCC_APB2ENR_SPI1EN; */
        *((uint32_t *)(0x42000000+(32*0x23844)+4*12)) = 0x00;
    } else if(spi == SPI2){
        /* bit banding address for SPI2 */
        gpio_reg = 0x42000000+(32*0x3800);
        *((uint32_t *)(gpio_reg + 24)) = 0x00;
        /* RCC->APB1ENR &= ~RCC_APB1ENR_SPI2EN; */
        *((uint32_t *)(0x42000000+(32*0x23840)+4*14)) = 0x00;
    } else {
        /* bit banding address for SPI3 */
        gpio_reg = 0x42000000+(32*0x3C00);
        *((uint32_t *)(gpio_reg + 24)) = 0x00;
        /* RCC->APB1ENR &= ~RCC_APB1ENR_SPI3EN; */
        *((uint32_t *)(0x42000000+(32*0x23840)+4*15)) = 0x00;
    }
